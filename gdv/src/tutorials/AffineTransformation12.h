#ifndef GDV_AFFINETRANSFORMATION12_H
#define GDV_AFFINETRANSFORMATION12_H
...
private:
    ...
    static void onClickSource(int event, int x, int y, int flags, void* param) {
        if (event != EVENT_LBUTTONDBLCLK || m_sourcePoints.size() == 3) {
            return;
        }
    ...
    static void onClickDest(int event, int x, int y, int flags, void* param) {
        if (event != EVENT_LBUTTONDBLCLK || m_destPoints.size() == 3) {
            return;
        }
...
public:
    ...
    [[noreturn]] void run() {
        while (true) {
            if (!m_computationDone && m_sourcePoints.size() == 3 && m_destPoints.size() == 3) {
                Mat transformMat = getAffineTransform(m_sourcePoints, m_destPoints);
                warpAffine(m_originalImg, m_transformImg, transformMat, Size(m_originalWidth, m_originalHeight));
...
#endif //GDV_AFFINETRANSFORMATION12_H
